...
export const ADMIN_TABLES: AdminTableDef[] = [
  ...
  },
  {
    ...
    pk: 'id',
    fields: [
      { key: 'title_top', label: 'Tiêu đề thanh trên', type: 'text', required: true },
      { key: 'link_top', label: 'Link thanh trên (để trống = chưa gắn link)', type: 'text' },
      { key: 'image_url', label: 'Ảnh giữa khối', type: 'image' },
      { key: 'title_bottom', label: 'Tiêu đề thanh dưới (để trống = ẩn thanh dưới)', type: 'text' },
      { key: 'link_bottom', label: 'Link thanh dưới', type: 'text' },
      { key: 'display_order', label: 'Thứ tự', type: 'number' },
      { key: 'is_active', label: 'Kích hoạt', type: 'checkbox' },
    ],
    cols: [
      { key: 'id', label: 'ID' },
      { key: 'image_url', label: 'Ảnh', render: thumb },
      { key: 'title_top', label: 'Thanh trên' },
      { key: 'title_bottom', label: 'Thanh dưới' },
      { key: 'display_order', label: 'Thứ tự' },
      { key: 'is_active', label: 'Active', render: boolBadge },
    ],
  },
